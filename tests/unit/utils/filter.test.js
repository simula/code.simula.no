import { describe, it, expect } from 'vitest'
import {
    VALID_SORTS,
    readFilterState,
    filterRepos,
    sortRepos
} from '../../../src/utils/filter'

describe('VALID_SORTS', () => {
    it('exposes the three supported sort keys', () => {
        expect(VALID_SORTS).toEqual(['recent', 'az', 'za'])
    })
})

describe('readFilterState', () => {
    it('returns defaults for empty/undefined input', () => {
        expect(readFilterState('')).toEqual({
            q: '',
            facets: { domain: [], type: [], language: [] },
            sort: 'recent'
        })
        expect(readFilterState(undefined)).toEqual({
            q: '',
            facets: { domain: [], type: [], language: [] },
            sort: 'recent'
        })
    })

    it('parses ?q=foo into the q field', () => {
        expect(readFilterState('?q=hello').q).toBe('hello')
    })

    it('accepts a query string without the leading ?', () => {
        expect(readFilterState('q=hello').q).toBe('hello')
    })

    it('decodes URL-encoded values', () => {
        expect(readFilterState('?q=hello%20world').q).toBe('hello world')
    })

    it('collects multiple values per facet param into an array', () => {
        const { facets } = readFilterState(
            '?domain=networking&domain=systems&type=cli-tool'
        )
        expect(facets.domain).toEqual(['networking', 'systems'])
        expect(facets.type).toEqual(['cli-tool'])
        expect(facets.language).toEqual([])
    })

    it('returns empty arrays per facet when no facet params are present', () => {
        const { facets } = readFilterState('?q=foo')
        expect(facets).toEqual({ domain: [], type: [], language: [] })
    })

    it('accepts each valid sort value', () => {
        for (const s of VALID_SORTS) {
            expect(readFilterState(`?sort=${s}`).sort).toBe(s)
        }
    })

    it('falls back to "recent" for an invalid sort value', () => {
        expect(readFilterState('?sort=garbage').sort).toBe('recent')
    })

    it('parses combined q + facets + sort together', () => {
        expect(
            readFilterState(
                '?q=cats&domain=networking&type=cli-tool&language=cpp&sort=az'
            )
        ).toEqual({
            q: 'cats',
            facets: {
                domain: ['networking'],
                type: ['cli-tool'],
                language: ['cpp']
            },
            sort: 'az'
        })
    })
})

describe('filterRepos', () => {
    const r = (slug, frontmatter) => ({ slug, frontmatter })
    const sample = [
        r('alpha', {
            title: 'Alpha',
            desc: 'medical imaging python model',
            domain: ['medical-imaging'],
            type: ['model'],
            language: ['python']
        }),
        r('beta', {
            title: 'Beta',
            desc: 'networking cli tool',
            domain: ['networking'],
            type: ['cli-tool'],
            language: ['cpp']
        }),
        r('gamma', {
            title: 'Gamma',
            desc: 'pure shell scripts',
            domain: [],
            type: ['scripts'],
            language: ['shell']
        }),
        r('delta', {
            title: 'Delta',
            desc: 'kitchen sink',
            domain: ['medical-imaging', 'networking'],
            type: ['model'],
            language: ['python']
        })
    ]

    const noFacets = { domain: [], type: [], language: [] }

    it('returns all repos when no filters are active', () => {
        const result = filterRepos({
            repos: sample,
            search: '',
            facets: noFacets
        })
        expect(result.map(d => d.slug)).toEqual([
            'alpha',
            'beta',
            'gamma',
            'delta'
        ])
    })

    it('treats undefined search/facets the same as empty', () => {
        const result = filterRepos({ repos: sample })
        expect(result).toHaveLength(sample.length)
    })

    it('treats multiple values within a facet as OR', () => {
        const result = filterRepos({
            repos: sample,
            facets: { ...noFacets, domain: ['medical-imaging', 'networking'] }
        })
        // All repos with EITHER domain (alpha=medical, beta=networking,
        // delta=both). Gamma has no domain so it's excluded.
        expect(result.map(r => r.slug).sort()).toEqual([
            'alpha',
            'beta',
            'delta'
        ])
    })

    it('treats selections across facets as AND', () => {
        const result = filterRepos({
            repos: sample,
            facets: {
                domain: ['medical-imaging'],
                type: ['model'],
                language: []
            }
        })
        // Need medical-imaging in domain AND model in type. Alpha and delta qualify.
        expect(result.map(r => r.slug).sort()).toEqual(['alpha', 'delta'])
    })

    it('returns no results when an across-facet AND matches nothing', () => {
        const result = filterRepos({
            repos: sample,
            facets: {
                domain: ['medical-imaging'],
                type: ['scripts'],
                language: []
            }
        })
        expect(result).toEqual([])
    })

    it('matches search against the title (case-insensitive)', () => {
        const result = filterRepos({
            repos: sample,
            search: 'ALPHA',
            facets: noFacets
        })
        expect(result.map(r => r.slug)).toEqual(['alpha'])
    })

    it('matches search against the description', () => {
        const result = filterRepos({
            repos: sample,
            search: 'cli',
            facets: noFacets
        })
        expect(result.map(r => r.slug)).toEqual(['beta'])
    })

    it('matches search against tag names across all facets', () => {
        const result = filterRepos({
            repos: sample,
            search: 'shell',
            facets: noFacets
        })
        // Gamma has language=shell. Also any other repo whose
        // title/desc/tags contain "shell" — gamma's desc also matches.
        expect(result.map(r => r.slug)).toEqual(['gamma'])
    })

    it('trims surrounding whitespace from the search term', () => {
        const result = filterRepos({
            repos: sample,
            search: '   alpha   ',
            facets: noFacets
        })
        expect(result.map(r => r.slug)).toEqual(['alpha'])
    })

    it('combines search and facets (both must match)', () => {
        const result = filterRepos({
            repos: sample,
            search: 'kitchen',
            facets: { ...noFacets, type: ['model'] }
        })
        expect(result.map(r => r.slug)).toEqual(['delta'])
    })

    it('handles repos without a desc field gracefully', () => {
        const noDesc = [{ slug: 'x', frontmatter: { title: 'No Desc Here' } }]
        expect(
            filterRepos({
                repos: noDesc,
                search: 'desc',
                facets: noFacets
            }).map(r => r.slug)
        ).toEqual(['x'])
        expect(
            filterRepos({
                repos: noDesc,
                search: 'unrelated',
                facets: noFacets
            })
        ).toEqual([])
    })
})

describe('sortRepos', () => {
    const mk = (slug, title, mtime) => ({
        slug,
        frontmatter: { title, mtime }
    })

    it('does not mutate the input array', () => {
        const input = [
            mk('a', 'B', '2024-02-01T00:00:00Z'),
            mk('b', 'A', '2024-01-01T00:00:00Z')
        ]
        const before = input.map(r => r.slug)
        sortRepos(input, 'az')
        expect(input.map(r => r.slug)).toEqual(before)
    })

    it('sorts a-z by title alphabetically', () => {
        const input = [
            mk('1', 'Charlie', '2024-01-01T00:00:00Z'),
            mk('2', 'Alpha', '2024-01-01T00:00:00Z'),
            mk('3', 'Bravo', '2024-01-01T00:00:00Z')
        ]
        expect(
            sortRepos(input, 'az').map(r => r.frontmatter.title)
        ).toEqual(['Alpha', 'Bravo', 'Charlie'])
    })

    it('sorts z-a by title in reverse', () => {
        const input = [
            mk('1', 'Alpha', '2024-01-01T00:00:00Z'),
            mk('2', 'Charlie', '2024-01-01T00:00:00Z'),
            mk('3', 'Bravo', '2024-01-01T00:00:00Z')
        ]
        expect(
            sortRepos(input, 'za').map(r => r.frontmatter.title)
        ).toEqual(['Charlie', 'Bravo', 'Alpha'])
    })

    it('uses localeCompare for non-ASCII titles', () => {
        // Æ should sort after A but before B in Norwegian/most locales —
        // but the exact pairing varies by locale. The key invariant we test
        // is that localeCompare is used (not bytewise compare), so Z vs Å
        // ordering is locale-aware rather than codepoint-ordered.
        const input = [
            mk('1', 'Z-thing', '2024-01-01T00:00:00Z'),
            mk('2', 'Æ-thing', '2024-01-01T00:00:00Z'),
            mk('3', 'A-thing', '2024-01-01T00:00:00Z')
        ]
        const titles = sortRepos(input, 'az').map(r => r.frontmatter.title)
        expect(titles[0]).toBe('A-thing')
    })

    it('sorts "recent" by mtime descending', () => {
        const input = [
            mk('old', 'X', '2023-01-01T00:00:00Z'),
            mk('newest', 'Y', '2024-06-01T00:00:00Z'),
            mk('mid', 'Z', '2024-01-01T00:00:00Z')
        ]
        expect(sortRepos(input, 'recent').map(r => r.slug)).toEqual([
            'newest',
            'mid',
            'old'
        ])
    })

    it('falls back to "recent" sort for unknown sort values', () => {
        const input = [
            mk('old', 'X', '2023-01-01T00:00:00Z'),
            mk('newest', 'Y', '2024-06-01T00:00:00Z')
        ]
        expect(sortRepos(input, 'whatever').map(r => r.slug)).toEqual([
            'newest',
            'old'
        ])
    })
})
