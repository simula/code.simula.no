...
import { FiArrowLeft, FiExternalLink, FiGithub, FiBookOpen } from 'react-icons/fi'
import RepoCard from '../components/repo-card'
import { allTagsFor, findRelatedRepos, formatMonthYear } from '../utils'
import { loadAllRepos, loadRepo } from '../utils/repos'
import { renderAndSanitize } from '../utils/markdown'
import { FACETS, TAG_LABEL } from '../data/tags'

const SITE_URL = 'https://code.simula.no'

export async function getStaticPaths() {
    const paths = loadAllRepos().map(({ slug }) => ({ params: { slug } }))
    return {
        paths,
        fallback: false
    }
...
export async function getStaticProps({ params: { slug } }) {
    const { frontmatter, content } = loadRepo(slug)

    const html = renderAndSanitize(content)

    const related = findRelatedRepos(
        { slug, frontmatter },
        loadAllRepos(),
        3
...
export default function RepoPage({ slug, frontmatter, html, related }) {
    ...
    const jsonLd = {
        ...
        ...(frontmatter.language?.length > 0 && {
            programmingLanguage: frontmatter.language
        }),
    ...
    return (
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <Head>
                <title>Simula Code - {frontmatter.title}</title>
