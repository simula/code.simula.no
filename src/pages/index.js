...
import { loadAllRepos } from '../utils/repos'
...
export async function getStaticProps() {
    ...
    // the full markdown body of every repo would bloat the static props.
    const repos = loadAllRepos().map(({ slug, frontmatter }) => ({
        slug,
        frontmatter
    }))

    const visibleRepos = repos.filter(r => !r.frontmatter.hidden)
    const facetCounts = countFacets(visibleRepos)
...
export default function Home({ repos, facetCounts }) {
    const router = useRouter()
    const searchInputRef = useRef(null)

    // Read URL filters synchronously on the first client render so a
    // direct hit on /?domain=networking paints the filtered grid in one pass
    ...
    const visibleRepos = useMemo(
        () => repos.filter(r => !r.frontmatter.hidden),
        [repos]
    )
    ...
    const filteredRepos = useMemo(
        ...
        [visibleRepos, searchValue, selectedFacets, sort]
    )
    ...
    return (
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
            <Head>
                <title>Simula Code</title>
                <meta
                    name="description"
                    content={`Browse ${visibleRepos.length} open-source code repositories published by Simula Research Laboratory and SimulaMet.`}
                />
            </Head>
            ...
            <div className="mb-6 text-center sm:mb-8">
                ...
                <p className="mx-auto mt-4 max-w-2xl text-base text-gray-600 sm:mt-5 sm:text-lg">
                    Browse {visibleRepos.length} open-source code repositories published by
            ...
            <div className="sticky top-0 z-10 -mx-4 mb-6 bg-white/95 px-4 py-3 backdrop-blur sm:-mx-6 sm:px-6 sm:py-4">
                ...
                <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-gray-500">
                    <p role="status" aria-live="polite">
                        Showing <span className="font-medium text-gray-900">{filteredRepos.length}</span> of{' '}
                        {visibleRepos.length} repositories
                        {totalSelected > 0 && (
                            <>
                                {' '}filtered by{' '}
                                {FACETS.flatMap(f =>
