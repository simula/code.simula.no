...
const CATEGORY_TO_FACET = {
    domain: 'domain',
    type: 'type',
    language: 'language'
}

function RepoCard({ repo, onTagClick }) {
    const { slug, frontmatter } = repo
    // Domain first, then type, then language — most descriptive first.
    const tags = [
        ...(frontmatter.domain || []),
        ...(frontmatter.type || []),
                ...
                <div className="flex items-center gap-1">
                    ...
                    {frontmatter.homepage && (
                        <a
                            ...
                            aria-label={`Visit homepage for ${frontmatter.title}`}
                            title="Homepage"
                            className="rounded-sm p-1.5 text-gray-500 transition-colors hover:bg-gray-100 hover:text-primary focus:outline-hidden focus-visible:ring-2 focus-visible:ring-primary"
                        >
